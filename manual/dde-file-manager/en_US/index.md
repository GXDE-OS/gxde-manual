# GXDE File Manager|../common/deepin-file-manager.svg|

## Overview

GXDE File Manager is developed by Deepin Technology team. As the core component to manage files in deepin, it is a powerful, easy-to-use file manager with an elegant and simple interface. Full-featured, well-designed, efficient-improved, GXDE File Manager will be a good help in your daily life and work.

![1|overview](jpg/overview.jpg)

## Guide

You can run, close and create a shortcut for GXDE File Manager by the following ways.

### Run GXDE File Manager

1. Click ![Launcher](icon/launcher_icon.svg) on dock to enter Launcher interface.
2. Locate ![File Manager](icon/deepin-file-manager.svg) by scrolling mouse wheel or searching by "GXDE File Manager" in Launcher interface.
...
> ![tips](icon/tips.svg): In Control Center, you can set GXDE File Manager as the default file manager. Specific operations refer to [Default Application Settings](dman:///dde#Default Application Settings).

### Exit GXDE File Manager

- On GXDE File Manager interface, click on ![close](icon/close_icon.svg) to exit.
- Right click on ![File Manager](icon/deepin-file-manager.svg) on Dock, select **Close All** to exit GXDE File Manager.
- Click on ![Settings](icon/icon_menu.svg) and select **Exit** to exit GXDE File Manager.

## Main Interface

On GXDE File Manager interface, users can execute many operations with the functional bars. It is designed to be easy-to-use, user-friendly and efficient improving.
...
GXDE File Manager is featured with the basic functions as a file manager, user can easily create, copy, rename or delete files/folders with it.

### New Document/Folder

#### New Folder

1. On GXDE File Manager interface, right click and select **New folder**.
...
1. On GXDE File Manager interface, right click and select **New document**.
...
1. On GXDE File Manager, right click on the file to open.
...
Click on ![icon](icon/icon_view_normal.svg) or ![list](icon/list_view_normal.svg) on GXDE File Manager interface to switch between icon view and list view.
...
1. Right click in the blank area on GXDE File Manager, select **Sort by**.
...
1. On GXDE File Manager interface, right click on a file/folder or more and select **Properties**.
...
GXDE File Manager is featured with many useful functions, such as breadcrumb bar, tabs, bookmarks, file sharing, accessing computers in LAN, opening file/folder in Terminal and so on. They are all designed to ease the commonly used operations.
...
GXDE File Manager supports multi-tab view.

1. On GXDE File Manager, right click on the folder to open in a new tab.
...
> ![tips](icon/tips.svg): When the GXDE File Manager has only one tab, the tab bar is hidden, you can press ![Ctrl](icon/Ctrl.svg) + ![T](icon/T.svg) to add a new tab.
...
Users can add folders to bookmarks in GXDE File Manager.

1. On GXDE File Manager, right click on the folder to add as bookmark.
...
GXDE File Manager supports file preview. Select a file and press the space bar on the keyboard to quickly preview the file info, including image resolution, document size, text content, gif, audio and video playback, etc.
...
1. On GXDE File Manager interface, click on ![network](icon/network_server.svg).
...
Users can share folders in GXDE File Manager, if there are some folders shared, the share icon will show on the navigation bar, if not, the share icon will be hidden.

1. On GXDE File Manager interface, right click on a folder, select **Properties**.
...
Users can manage shared folders if it has shared some folders in GXDE File Manager.

1. On GXDE File Manager interface, click ![share](icon/icon_share.svg) on the navigation bar.
...
1. On GXDE File Manager interface, enter the folder.
...
1. On GXDE File Manager interface, select the item to create a link for.
...
On GXDE File Manager interface, you can mount/unmount, remove, format and decrypt disks.

### Display Disks

Mounted disks are displayed in the left navigation bar of GXDE File Manager. Including:
...
1. In the navigation bar of GXDE File Manager, right click on the disk.
...
1. In the navigation bar of GXDE File Manager, right click on the disk to format.
2. Select **Format**.
...
1. On GXDE File Manager interface, click on ![menu](icon/icon_menu.svg).
2. Select **New window** to open a new window.

### Dark Theme

1. On GXDE File Manager interface, click on ![main_menu](icon/icon_menu.svg).
2. Select **Dark Theme**.
...
3. Modify the settings according to your needs:
 ...
 - In **Dialog**, users can set whether to use the file chooser dialog of the GXDE File Manager.
